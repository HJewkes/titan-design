...
export function over(base: string, a: number, tint: [number, number, number] = [255, 255, 255]): string {
...
export function PaperCollage({ tones, desk, accent }: { tones: string[]; desk: string; accent: string }) {
  return (
    <View
      style={
        {
...
export function TopBarDemo({ label, barStyle, spine }: { label: string; barStyle: object; spine?: boolean }) {
  const c = getSemanticColors('dark')
  return (
    <View style={{ gap: 8 }}>
...
export function recessShadow(o: number, b: number, sp: number, a: number, la: number, rim: number): string {
  return [
