    ...
    <div style={{ display: 'flex', width: '100%', height: h, borderRadius: 2, overflow: 'hidden' }}>
      {chunks.map((ch, ci) => (
        <Fragment key={ci}>
          ...
          {ci < chunks.length - 1 && (
            <div
              style={{ flex: `0 0 ${ch.gapAfter ?? NOTCH}px`, height: '100%', background: INSET }}
            />
          )}
        </Fragment>
      ))}
...
function DropBar({ subloads, h }: { subloads: number[][]; h: number }) {
  const chunks: Chunk[] = subloads.map((vels, i) => ({
    colors: vels.map(velColor),
    gapAfter: NOTCH,
    ...(i === subloads.length - 1 ? { gapAfter: undefined } : {}),
  }))
...
function MyoBar({
  ...
  const chunks: Chunk[] = [
    { colors: activation.map(velColor), gapAfter: CLUSTER_GAP },
    ...clusters.map((c) => ({ colors: c.map(velColor), gapAfter: CLUSTER_GAP })),
  ]
  return <Bar chunks={chunks} h={h} />
}
    ...
    <div style={{ display: 'flex', width: '100%', height: h, gap: 0 }}>
      ...
      {fades.map((op, ci) => (
        <Fragment key={ci}>
          ...
          <div
            style={{ display: 'flex', flex: 5, height: '100%', overflow: 'hidden', opacity: op }}
          >
...
function Sheet() {
  return (
    <Page
      title="Session Rail — set-type encodings"
      note="R&D for prescriptions the flat sets×reps strip doesn't model. Rail width, real velocity ramp pins. Rep-range: range-max segments, the variable zone (floor→max) upcoming part gets its own color. Drop: one bar, sub-loads split by thin notches. Myo: activation + mini-clusters split by cluster gaps."
    >
      <div style={{ display: 'flex', gap: 40, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        ...
        <Col label="1 · variable rep-range — LOCKED: cyan 900">
          {(
            ...
            <Row key={label} name="Face Pull" meta="3 × 15–20 @ 40">
              <span style={{ fontSize: 9, color: T_TERTIARY, display: 'block', marginBottom: 5 }}>
                {label}
              </span>
          ...
          <span style={{ fontSize: 10, color: T_TERTIARY, maxWidth: RAIL_W }}>
            Each block: todo · 12 done (committed) · 17 done (into the variable zone). Variable-todo
            = the swatch; committed-todo = grey.
          </span>
        </Col>
        ...
        <Col label="2 · drop set">
          ...
          <Row name="Leg Extension" meta="1 drop · 12→10→8→6 @ 90→45">
            <DropBar
              subloads={[reps(12, 0.8), reps(10, 0.7), reps(8, 0.62), reps(6, 0.55)]}
              h={H}
            />
          </Row>
          <span style={{ fontSize: 10, color: T_TERTIARY, maxWidth: RAIL_W }}>
            One set, no rest; notches mark each load drop. Notch (2px) &lt; set gap (5px) so drops
            read as sub-structure of a single set.
          </span>
        </Col>
        ...
        <Col label="3 · myo-rep / rest-pause">
          <Row name="Cable Curl" meta="myo · done · 15 + 5 + 5 + 5">
            <MyoBar
              activation={reps(15, 0.75)}
              clusters={[reps(5, 0.6), reps(5, 0.55), reps(5, 0.5)]}
              h={H}
            />
          </Row>
          ...
          <span style={{ fontSize: 10, color: T_TERTIARY, maxWidth: RAIL_W }}>
            Done: activation to near-failure, then rest-pause clusters. Cluster gap (3px) between
            notch and set gap.
          </span>
          <span style={{ ...sectionTitle, color: T_SECONDARY, marginTop: 6 }}>
            upcoming · length unknown
          </span>
          ...
          <span style={{ fontSize: 10, color: T_TERTIARY, maxWidth: RAIL_W }}>
            {
              'Grey activation (expected) + blue "clusters-to-failure" trail that fades and leaves the right edge open — reuses the variable/opportunity blue, signalling "more coming, count unknown."'
            }
          </span>
        </Col>
      </div>
